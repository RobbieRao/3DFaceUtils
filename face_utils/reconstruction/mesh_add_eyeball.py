...
#https://github.com/cleardusk/3DDFA_V2/blob/d585db4a79538152fbb6ca09db7bf5b9f1c9a63e/utils/uv.py
#get_colors
        ...
        if line.startswith('v '):
            ...
            if len(toks) == 4:  # quad faces
                ...
                if len(faces1) >= 2:
                    ...
                    face_vt.append(np.array([faces1[1], faces2[1], faces3[1]], np.int32) - 1)
                    face_vt.append(np.array([faces1[1], faces3[1], faces4[1]], np.int32) - 1)
                if len(faces1) >= 3:
                    if len(faces1[2]) == 0:
                        continue
...
def add_eye_ball(head_vtx, L_ball_mesh=None, R_ball_mesh=None):
    if L_ball_mesh is None:
        ...
        img=cv2.imread(ball_tex_path)[:,:,::-1]
        img_h,img_w,_=img.shape

        #N_point= R_ball_mesh["v"].shape[0]
        #L_ball_mesh["vc"]=bilinear_interpolate(img,L_ball_mesh["vt"][:,1]*img_h,L_ball_mesh["vt"][:,0]*img_w)[:N_point,:]
        #R_ball_mesh["vc"]=bilinear_interpolate(img,R_ball_mesh["vt"][:,1]*img_h,R_ball_mesh["vt"][:,0]*img_w)[:N_point,:]
        
        tr_uv=L_ball_mesh["vt"][np.array(L_ball_mesh["fvt"]).flatten()]
        tr_uv[:,1]=1-tr_uv[:,1]
        ball_tricol=bilinear_interpolate(img,tr_uv[:,0]*img_h,tr_uv[:,1]*img_w)
        L_ball_col=np.ones_like(L_ball_mesh["v"])
        L_ball_col[L_ball_mesh["tri_v"].flatten(),:]=ball_tricol 
        L_ball_mesh["vc"]=L_ball_col.copy()

        tr_uv=R_ball_mesh["vt"][np.array(R_ball_mesh["fvt"]).flatten()]
        tr_uv[:,1]=1-tr_uv[:,1]
        ball_tricol=bilinear_interpolate(img,tr_uv[:,0]*img_h,tr_uv[:,1]*img_w)
        R_ball_col=np.ones_like(R_ball_mesh["v"])
        R_ball_col[L_ball_mesh["tri_v"].flatten(),:]=ball_tricol 
        R_ball_mesh["vc"]=R_ball_col.copy()
...
if __name__ == '__main__':
    ...
    # parser.add_argument(
    #     mesh_path="./eye_ball_assets/mesh_head.obj",
    #     type=str,
    #     required=True,
    ...
    head_mesh = obj_read_quad_tri(mesh_path)
    L_ball_mesh = obj_read_quad_tri(L_ball_mesh_path)
    R_ball_mesh = obj_read_quad_tri(R_ball_mesh_path)
