...
filepath='./Data/Children_head_Database/Tina/scanned_data/'
file_lists=[ i[:-4] for i in   os.listdir(filepath)]
#filePath1 ="./Data/Children_head_Database/Tina/temp_Fitting_Face/"
#file_lists0=os.listdir(filePath1)  
#file_idx=[61,207,213]
#==================================
#for idx in file_idx:#:range(23,len(file_lists0)):
idx=128
#==================================
filenum="y0039m"#file_lists[idx]#[:-4]
filename=filepath+filenum+'.ply' #
print(idx," ",filename)

T_vertices0,T_colors0,T_triangles0= read_ply(filename)
T_vertices0=T_vertices0*1000  #unit: mm

angle0=np.array([-5,0, 0])
if (angle0==0).sum()!=0:
...
Sub_vertices[Sub_vertices[:,1]>60,0]=0
Sub_vertices[Sub_vertices[:,1]<0,0]=0

Ear_idx=np.argmax(np.abs(Sub_vertices[:,0]))

Threshold_ear=np.sqrt(np.sum(Sub_vertices[Ear_idx,:]**2))*1.08

weight1=np.array([1,2.5,1]).reshape(1,3)
...
# ============================================Face Pose Corection ==========================================
#T_vertices2=T_vertices1
#T_vertices0r=T_vertices0
...
#save_ply(T_vertices0r[:,0:3],T_colors0,T_triangles0,'./Data/Corrected_Heads/Corrected_Head'+filenum+'.ply')
#try:
detect_method="baidu" #face_alignment
...
aligned =correspond_mesh(source,target,group,
                         landmark_weights=[10,5,2,1,0.0,0.0, 0.00,0.0],
                         stiffness_weights = [50, 20, 5, 2, 0.8, 0.5, 0.35, 0.2],
                         PerNrom_type="Percentage",Para_Nrom=0.3,
...
land_idx=np.r_[X_ind,S_Ear_idx1,S_Ear_idx2]#C['model']['kpt_ind'][0,0].ravel(),S_Ear_idx1,S_Ear_idx2]
...
filepath1='./Data/Children_head_Database/Tina/'
save_ply(T_vertices0r[:,0:3],T_colors0,T_triangles0,filepath1+"Corrected_Head/"+filenum+'.ply')
save_ply(F_vertices,S_colors*255,F_triangles,filepath1+"temp_Fitting_Face/"+filenum+'.ply')
# #------------------------------------------------Fitting Error--------------------------------------------
...
# #save_ply(F_vertices, S_colors*255,  F_triangles,'./Data/Fitting_Faces/Fitting_Face'+filenum+'.ply')
# #save_ply(T_vertices2,T_colors1,T_triangles2,'./Data/Corrected_Faces/Corrected_Face'+filenum+'.ply')
