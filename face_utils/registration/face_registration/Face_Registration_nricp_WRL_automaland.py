...
S_landmarks=S_vertices[X_ind[land_select],:]
...
manual_land=df_landmark.loc[row_idx,['Nose','Tragion left','Tragion right']]
...
T_vertices0=T_vertices0-T_vertices0[Tragion_idx[0],:].reshape(1,3)
...
Threshold_ear=np.max([np.sqrt(np.sum(T_vertices0[Tragion_idx[1],:]**2)),
                      np.sqrt(np.sum(T_vertices0[Tragion_idx[2],:]**2))])*1.35
...
T_landmarks=T_vertices2[np.r_[T_landmark_idx[np.arange(17,68)],Tragion_idx1[1:3]],:]
